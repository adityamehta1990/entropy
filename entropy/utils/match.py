...
def matchOneIdentifier(query, choices, scorer=fuzz.token_set_ratio):
    '''returns fuzzy matched identifier if unique, else None'''
    matches = process.extractBests(query, choices, scorer=scorer, score_cutoff=FUZZ_SCORE_CUTOFF)
    return matches[0][0] if len(matches) == 1 else None

def matchAnyIdentifier(query, choices, scorer=fuzz.token_set_ratio):
...
def matchDictBest(query, choices, key=None, scorer=fuzz.token_set_ratio):
    ...
    matches = [c for c in choices if scorer(query[key], c[key]) > FUZZ_SCORE_CUTOFF]
    if len(matches) == 1:
        return matches[0]
    elif len(matches) > 1:
